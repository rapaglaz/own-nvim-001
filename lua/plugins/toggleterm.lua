return {
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader><F3>", function() float_term:toggle() end, { desc = "Toggle float terminal" })

      -- Vertical terminal (F4)
      local vertical_term = Terminal:new({ direction = "vertical", size = 50, hidden = true })
      vim.keymap.set("n", "<leader><F4>", function() vertical_term:toggle() end, { desc = "Toggle vertical terminal" })

      -- Git terminal (F5)
      local git_term = Terminal:new({ cmd = "lazygit", direction = "float", hidden = true })
      vim.keymap.set("n", "<leader><F5>", function() git_term:toggle() end, { desc = "Toggle LazyGit" })
